...
describe('CalculadoraService', () => {
  ...
  it('Deve gatantir que 5 - 3 = 2',
    inject([CalculadoraService], (service: CalculadoraService) => {
      const subtracao = service.calcular(5, 3, CalculadoraService.SUBTRACAO);
      expect(subtracao).toEqual(2);
  }));

  it('Deve garantir que 12 / 2 = 6',
    inject([CalculadoraService], (service: CalculadoraService) => {
      const divisao = service.calcular(12, 2, CalculadoraService.DIVISAO);
  ...
  it('Deve retornar 0 para operação invalida',
    inject([CalculadoraService], (service: CalculadoraService) => {
      const opInvalida = service.calcular(2, 3, '%');
